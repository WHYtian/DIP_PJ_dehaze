...
from model_change3 import DM2FNet
...
def train(net, optimizer):
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        loss_x_j5_record, loss_t_record, loss_a_record = AvgMeter(), AvgMeter(), AvgMeter()
...
def validate(net, curr_iter, optimizer):
    ...
    with torch.no_grad():
        for data in tqdm(val_loader):
            ...
            x_jf, x_j0, x_j1, x_j2, x_j3, x_j4, x_j5= net(haze)
    ...
    log = '[validate]: [iter {}], [loss {:.5f}] [PSNR {:.4f}] [SSIM {:.4f}] [CIEDE2000 {:.4f}], [loss_x_phy {:.5f}], [loss_x_j1 {:.5f}], ' \
                  '[loss_x_j2 {:.5f}], [loss_x_j3 {:.5f}], [loss_x_j4 {:.5f}], [loss_x_j5 {:.5f}]'.format(
