...
def train(net, optimizer):
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        for data in train_loader:
            ...
            x_jf, x_j0, x_j1, x_j2, x_j3, t, a = net(haze)
            ...
            loss = loss_x_jf + loss_x_j0 + 2*loss_x_j1 + loss_x_j2 + loss_x_j3 \
                   + 10 * loss_t + loss_a
            loss.backward()
            ...
            log = '[iter %d], [train loss %.5f], [loss_x_fusion %.5f], [loss_x_phy %.5f], [loss_x_j1 %.5f], ' \
                  '[loss_x_j2 %.5f], [loss_x_j3 %.5f], [loss_t %.5f], [loss_a %.5f], ' \
                  '[lr %.13f]' % \
                  (curr_iter, train_loss_record.avg, loss_x_jf_record.avg, loss_x_j0_record.avg,
                   loss_x_j1_record.avg, loss_x_j2_record.avg, loss_x_j3_record.avg,
...
def validate(net, curr_iter, optimizer):
    ...
    with torch.no_grad():
        for data in tqdm(val_loader):
            ...
            x_jf, x_j0, x_j1, x_j2, x_j3 = net(haze)
    ...
    log = '[validate]: [iter {}], [loss {:.5f}] [PSNR {:.4f}] [SSIM {:.4f}] [CIEDE2000 {:.4f}], [loss_x_phy {:.5f}], [loss_x_j1 {:.5f}], ' \
                  '[loss_x_j2 {:.5f}], [loss_x_j3 {:.5f}]'.format(
