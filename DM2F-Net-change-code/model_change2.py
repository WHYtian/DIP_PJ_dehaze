...
class DM2FNet(Base):
    ...
    def forward(self, x0, x0_hd=None):
        ...
        x_j2 = ((x + 0.1*x_enhacer + r2) * self.std + self.mean).clamp(min=0., max=1.)
        ...
        return x_fusion, x_phy, x_j1, x_j2, x_j3, x_j4, t, r1, r2, r3, r4
...
class DM2FNet_woPhy(Base_OHAZE):
    ...
    def forward(self, x0):
        ...
        x_p3 = torch.exp(-log_x_p3_0 + F.upsample(self.p3_1(f), size=x0.size()[2:], mode='bilinear')).clamp(min=0,
                                                                                                            max=1)
        x_p4 = F.upsample(self.p4(f), size=x0.size()[2:], mode='bilinear')
        x_enhance = self.contrast_enhancer(x0)
        x_p4 = ((x_enhance+x0)* self.std_out + self.mean_out).clamp(min=0., max=1.)
