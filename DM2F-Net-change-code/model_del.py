...
class DM2FNet(Base):
    ...
    def forward(self, x0, x0_hd=None):
        ...
        if self.training:
            return x_fusion, x_phy, x_j1, x_j2, x_j3, t, a.view(x.size(0), -1)
        else:
            return x_fusion, x_phy, x_j1, x_j2, x_j3
        #return x_fusion, x_phy, x_j1, x_j2, x_j3, t, r1, r2, r3
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        self.attentional_fusion = nn.Sequential(
            nn.Conv2d(num_features, num_features // 2, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features // 2, num_features // 2, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features // 2, 9, kernel_size=3, padding=1)
        )
    ...
    def forward(self, x0):
        ...
        x_fusion = torch.cat((torch.sum(F.softmax(attention_fusion[:, : 3, :, :], 1) * torch.stack(
            (x_p0[:, 0, :, :], x_p1[:, 0, :, :], x_p2[:, 0, :, :]), 1), 1, True),
                              torch.sum(F.softmax(attention_fusion[:, 3: 6, :, :], 1) * torch.stack((x_p0[:, 1, :, :],
                                                                                                     x_p1[:, 1, :, :],
                                                                                                     x_p2[:, 1, :, :]),
                                                                                                    1), 1, True),
                              torch.sum(F.softmax(attention_fusion[:, 6:, :, :], 1) * torch.stack((x_p0[:, 2, :, :],
                                                                                                   x_p1[:, 2, :, :],
                                                                                                   x_p2[:, 2, :, :]),
                                                                                                  1), 1, True)),
                             1).clamp(min=0, max=1)
        ...
        if self.training:
            return x_fusion, x_p0, x_p1, x_p2
        else:
            return x_fusion
